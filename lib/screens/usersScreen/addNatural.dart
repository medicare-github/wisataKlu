...
class AddNatural extends StatefulWidget {
  final bool isUpdating;
  AddNatural({@required this.isUpdating});

  @override
  _AddNaturalState createState() => _AddNaturalState();
}

class _AddNaturalState extends State<AddNatural> {
  ...
  void initState() {
    super.initState();
    AddNotifier addNotifier = Provider.of<AddNotifier>(context, listen: false);
    AuthNotifier authNotifier =
        Provider.of<AuthNotifier>(context, listen: false);

    if (addNotifier.currentAdd != null) {
      _currentAdd = addNotifier.currentAdd;
    } else {
    ...
    _imageUrl = _currentAdd.image;
    _currentAdd.emailUser = authNotifier.user.email;
    _currentAdd.idUser = authNotifier.user.uid;
    _currentAdd.kind = 'natural';
    _currentAdd.status = 'not accepted';
  }
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                              ...
                              image: DecorationImage(
                                  image:
                                      AssetImage("assets/images/natural.jpg"),
                                  fit: BoxFit.cover)),
                          child: Column(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: <Widget>[
                              Text(
                                "Submision Your Natural Tourism",
                                style: TextStyle(
                                  color: Colors.white,
  ...
  _saveTourism() async {
    ...
    if (!_formKey.currentState.validate()) {
      ...
      _currentAdd.lating =
          GeoPoint(_currentAdd.latitude, _currentAdd.longitude);

      print('form saved');

      await uploadTourismAndImage(
          _currentAdd, widget.isUpdating, _imageFile, _onTourismUploaded);
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        onSaved: (String value) {
          _currentAdd.ticket = int.parse(value);
        },
      ),
    );
  }
  ...
  Widget _buildTourismNameField() {
    return Padding(
      ...
      child: TextFormField(
        ...
        validator: (String value) {
          if (value.isEmpty) {
            return 'Tourism name is required';
          }

          if (value.length < 4 || value.length > 25) {
            return 'Tourism name must be betweem 4 and 25characters';
          }

          return null;
        },
        onSaved: (String value) {
          _currentAdd.name = value;
        },
      ),
    );
  }
            ...
            child: DropdownButton(
              ...
              onChanged: (String newValue) {
                setState(() {
                  ...
                  _currentAdd.subDistric = newValue;
                });
              },
            ),
          ),
  ...
  Widget _buildVillageField() {
    return Padding(
      padding: const EdgeInsets.only(bottom: 8),
      child: TextFormField(
        // initialValue: _currentAdd.village,
        decoration: InputDecoration(
            contentPadding: EdgeInsets.only(top: 16, bottom: 16),
            prefixIcon: Padding(
              padding: const EdgeInsets.only(left: 18, right: 18),
  ...
  Widget _buildDescriptionField() {
    return Padding(
      padding: const EdgeInsets.only(bottom: 8),
      child: TextFormField(
        // initialValue: _currentAdd.description,
        maxLines: 3,
        decoration: InputDecoration(
            contentPadding: EdgeInsets.only(top: 16, bottom: 16),
            prefixIcon: Padding(
      ...
      child: Column(
        children: <Widget>[
          TextFormField(
            ...
            onSaved: (String value) {
              _currentAdd.longitude = double.parse(value);
            },
          ),
          mapToggle
              ? Padding(
      ...
      child: Column(
        children: <Widget>[
          TextFormField(
            ...
            onSaved: (String value) {
              _currentAdd.latitude = double.parse(value);
            },
          ),
          mapToggle
              ? Padding(
